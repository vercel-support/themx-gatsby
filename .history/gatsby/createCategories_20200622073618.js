...
module.exports = async ({ actions, graphql }) => {
  ...
  await fetchCategories({ first: 100, after: null }).then(allCategories => {
    ...
    const countryTemplate = path.resolve(`./src/templates/categoryCountry.js`)
    const continentTemplate = path.resolve(
      `./src/templates/categoryContinent.js`
    )
    ...
    allCategories.map(category => {
      ...
      if (category.slug === "bestemmingen") {
        ...
        console.log(`create country category: ${category.uri}`)
        createPage({
          path: `${category.uri}`,
