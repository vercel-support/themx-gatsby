  ...
  await fetchCategories({ first: 100, after: null }).then(allCategories => {
    ...
    allCategories.map(category => {
      if (category.slug === "bestemmingen") {
        console.log(
          `create category: ${category.uri} Ancestors: ${category.ancestors} `
        )
        createPage({
          path: `${category.uri}`,
        ...
        // } else if (category.ancestors.includes("bestemmingen")) {
        ...
        console.log(
          `create category: ${category.uri} ancestors: ${category.uri}`
        )
        createPage({
          path: `${category.uri}`,
