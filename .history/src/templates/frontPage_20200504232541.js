...
import titleCardsStyle from "../styles/frontPage/titleCards.module.scss"
      ...
      <div className="container grid">
        {edges.map(edge => {
          ...
          return (
            <Link
              to={uri}
              className={`position-relative ${titleCardsStyle.card}`}
            >
              <h2 key={uri} className={`${titleCardsStyle.cardTitle}`}>
                {title}
              </h2>
              {featuredImage.imageFile && (
                <Img
                  ...
                  alt={title}
                  className={`position-absolute ${titleCardsStyle.cardImage}`}
                />
              )}
            </Link>
          )
