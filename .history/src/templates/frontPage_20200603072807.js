...
const FrontPage = ({ props, data }) => {
  ...
  const { title } = page
  const { edges } = posts
  const imageSourcesHeader = ({ data }) => [
    data.featuredImage.portrait.childImageSharp.fluid,
    {
      ...data.featuredImage.landscape.childImageSharp.fluid,
      media: `(min-width: 768px)`,
    },
  ]
  const imageSourcesAboutUs = ({ data }) => [
    data.aboutUs.featuredImage.imageFile.childImageSharp.fluid,
    {
      ...data.aboutUs.featuredImage.imageFile.childImageSharp.fluid,
      media: `(min-width: 768px)`,
    },
    ...
    <Layout>
      ...
      <section className="container grid grid-3x2 pb-60">
        {edges.map(edge => {
          ...
          const imageSourcesSixPosts = ({ data }) => [
      ...
      <section className="container py-60">
        <div className="d-flex justify-content-between">
          <Link to={europeCat.uri}>
            ...
            <Img fixed={data.europePng.imageFile.childImageSharp.fixed} />
          </Link>
          <Link to={asiaCat.uri}>
            <Typography component="h3" className="text-center">
              {asiaCat.name}
            </Typography>
            <Img fixed={data.asiaPng.imageFile.childImageSharp.fixed} />
          </Link>
          <Link to={oceaniaCat.uri}>
            <Typography component="h3" className="text-center">
              {oceaniaCat.name}
            </Typography>
            <Img fixed={data.oceaniaPng.imageFile.childImageSharp.fixed} />
          </Link>
        </div>
      </section>
    </Layout>
