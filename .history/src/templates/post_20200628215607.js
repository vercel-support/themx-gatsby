...
import wordPressUrl from "../../config.js"
    ...
    <Layout>
      ...
      <div className="content-layout container pt-100">
        {seo.breadcrumbs.map(breadcrumb => {
          return (
            <>
              {console.log(breadcrumb.url.replace(wordPressUrl[0], ""))}
              {console.log(breadcrumb.url)}
              {console.log(wordPressUrl[0])}
              {breadcrumb.text == "Home" ? (
                <Link to="/">
                  <FontAwesomeIcon icon={faHome} className="mx-8" />
                </Link>
              ) : (
                <Link to={breadcrumb.url.replace(wordPressUrl, "")}>
                  {breadcrumb.text}
                </Link>
              )}
              <FontAwesomeIcon icon={faPlane} className="mx-8" />
            </>
          )
        })}
        <SEO title={title} />
