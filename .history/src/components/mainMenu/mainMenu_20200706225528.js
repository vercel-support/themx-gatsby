...
const renderMenuItem = (item, generalSettingsUrl) => {
  let hasChild = false;
...
const renderChildMenu = item => {
  return <ul className="card-shadow navbar nav">{item.childItems.nodes.map(child => renderMenuItem(child))}</ul>;
};
        ...
        return (
          <Container>
            <Navbar expand="lg" className="d-flex">
              <Navbar.Brand href="/">
                <svg className="header-logo" viewBox="0 0 109 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
              ...
              <Navbar.Collapse id="basic-navbar-nav">
                <Nav className="mx-auto">
                  {menu.map(item => renderMenuItem(item, generalSettingsUrl, generalSettingsTitle))}
                </Nav>
                <Form inline className="mb-0">
                  <FormControl type="text" placeholder="Zoek..." className="mr-sm-2 bg-gray-800 border-0" />
                  <Button className="px-16 bg-transparent border-0">
