...
export default function CategoryCards({
  ...
  return (
    <section
      className={`container grid pb-60 ${className}`}
    >
      {data.wpgraphql.posts.nodes.map((node) => {
        ...
        const imageSourcesCategoryCards = [
          ...featuredImage.xsSm.childImageSharp
            .fluid,
          {
            ...featuredImage.mdLgXl
        ...
        return (
          <Link
            className='category-cards card-shadow'
            to={uri}
            key={id}
          >
            {featuredImage && (
