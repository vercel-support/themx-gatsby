...
const PostCard = ({ props }) => {
  return (
    <section className="container grid grid-3x2 pb-60">
      {props.map(edge => {
        const { title, uri, featuredImage, countryMeta, categories, id } = edge
        // const imageSource = countryMeta ? (countryMeta.featuredImageCategory)}
        let imageSourcesSixPosts = [
          countryMeta
            ? (countryMeta.featuredImageCategory.portrait.childImageSharp.fixed,
              {
